...
function getTopRatedMovies() {
    ...
    const render = renderMovies.bind({ title: 'Filmes com Melhores Classificação' })
    requestMovies(url, render, handleGeneralError);
}

function getTrendingMovies() {
    const url = generateMovieDBUrl('/trending/movie/day');
    const render = renderMovies.bind({ title: 'Têndencias de Filmes' })
    requestMovies(url, render, handleGeneralError);
}
...
function searchUpcomingMovies() {
    ...
    const render = renderMovies.bind({ title: 'Próximos Filmes' })
    requestMovies(url, render, handleGeneralError);
}
